...
#+begin_exercise
Here is another algorithm that plots $\{L(t)\}$. Explain how it works, and it difference with the previous algorithm.
...
#+begin_exercise
First a test: we set the vector of server capacities ~c=[1]~ so that we reduce our multi-server queue to a single-server queue.
1. Why do we only have to change  the dimension of $c$ to realize this?
2. Run the code and check that both simulations give the same result.
