...
To reduce the average cost, an $N$-policy\sidenote{\cref{ex:n-policies}} can be used, works as follows.
...
The server processes jobs until the system is empty again, switches off, and remains idle until $N$ new jobs have arrived, and so on.\sidenote{Thus, an $N$-policy \emph{controls} the server.}
...
\newthought{Suppose it costs} $K$ Euro to set up the server, independent of the time it has been idle, and each job charges $h$ Euros per unit time while in the system.
...
\newthought{As a first step,} we concentrate on the expected time $T(q)$ it takes to clear the system when there are $q$ jobs present and the server switches on.
...
\newthought{As an immediate} application of the above, let us rederive $\E\L = \rho/(1-\rho)$, i.e.,~\cref{eq:el}.
...
Clearly, $C(1)=\E{I} + \E{U} = 1/\lambda + T(1)$.\sidenote{\cref{ex:57}}
