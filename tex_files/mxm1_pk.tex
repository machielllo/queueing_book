...
\end{equation}
Since $Y=\lim_{t\to\infty} Y(t)/t = \E{\Ls^{B}}$, it follows from $Y=\lambda X$ that\sidenote{\cref{ex:100}}
...
\newthought{Rather than using} the time of an entire batch spending time at the server, as in the above use of the renewal-reward equation, we can also concentrate on the time spent by single items at the server.
