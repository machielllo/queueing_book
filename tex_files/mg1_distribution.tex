...
Note that, because the service times are i.i.d., $\{Y_k\}$ is a sequence of i.i.d.\ random variables.
...
We use a conditioning argument to find an expression for $\P{Y=j}$. Suppose we know that the service time is $x$. Then, $Y \sim P(\lambda x)$, hence, 
 \begin{equation*}
...
With this,
 \begin{equation*}
 \P{Y_k=j} = \int_0^\infty \P{Y_k =j\given S=x} F(\d x)
