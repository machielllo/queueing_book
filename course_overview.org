...
2. For each assignment you have to hand in a pdf file, which is typeset in /LaTeX/. You should use the template made available on github, and it should include the names and ids of both students, a title, and a date.
...
5. Assignment $n$ is  due at Friday 18h00 in week $n+1$ of the course. You are supposed to start working on the assignment during the tutorial.
