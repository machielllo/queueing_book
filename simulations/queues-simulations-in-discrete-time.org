#+title: Queueing Theory: Simulation in discrete time
...
By setting the seed we can guarantee to get the same samples every time.
...
#+begin_exercise
Comment on this graph. Did you expect such large excursions of the queue lenght process?
#+end_exercise
...
You should see that is is very different from the one before. To explain why this is so, this somehwat cryptic question will be helpful, hopefully.
...
#+BEGIN_exercise
Why is ~dtype=float~ not necessary in the definition of ~L~?
#+END_exercise
...
#+begin_exercise
Now change the parameters such  $\rho \approx 10$, use a simple argument to show that $\pi_{K-2} \approx 0$. Check this intuintuion by doing a simulation with the appropriate parameters. Include your code and the graph of $p$.
#+end_exercise
