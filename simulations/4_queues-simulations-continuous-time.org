...
* Alternative KPI computations
Here is another way to compute the KPIs.
Not only is this useful, it also helps to understand the concepts and definitions related to level-crossing arguments, see Section 5.2 of the book.

We give all code right away.
Then in the exercises we ask you to explain what the code does.
For this, run the code, check the output print statements, and then explain how the relevant line(s) code works
...
# strip fake departure at t = 0
D = D[1:]

a = np.zeros((len(A), 3))
a[:, 0] = A
a[:, 1] = 1
# There is no arrival at time 0
a[0, 1] = 0
print(a)
...
A_n = L[L[:, 1] == 1, 2] - 1
print(np.unique(A_n, return_counts=True))

D_n = L[L[:, 1] == -1, 2]
print(np.unique(D_n, return_counts=True))

durations = np.zeros((len(L) - 1, 2))
durations[:, 0] = L[1:, 0] - L[:-1, 0]
durations[:, 1] = L[:-1, 2]
print(durations)

Y = {}
for n in range(5):
    Y[n] = durations[durations[:, 1] == n].sum()

print(Y)
#+end_src

\begin{exercise}
What do lines 21 to 26 achieve?
\end{exercise}

\begin{exercise}
What do lines 28 to 31 achieve?
\end{exercise}

\begin{exercise}
What does line 34 achieve?
\end{exercise}

\begin{exercise}
What does line 37 achieve? How does it work?
\end{exercise}

\begin{exercise}
What does line 40 achieve?
\end{exercise}

\begin{exercise}
What do line 43 and 44 achieve? To which definition in the book does it relate?
\end{exercise}

\begin{exercise}
What do line 43 and 44 achieve? To which definition in the book does it relate? Why do we have to subtract 1 in line 43?
\end{exercise}

\begin{exercise}
What do line 46 and 47 achieve? To which definition in the book does it relate? Why don't we have to subtract 1?
\end{exercise}

\begin{exercise}
What do line 49 to 52 achieve? To which definition in the book does it relate?
\end{exercise}

\begin{exercise}
What do line 54 to 56 achieve? To which definition in the book does it relate?
\end{exercise}
