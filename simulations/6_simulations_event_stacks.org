...
\begin{exercise}
Read on the web what ~islice~ does, and explain it in your own words. Explain also why we need to turn it into a ~list~. (This requires that you read and think about what a /generator/ is.)
\end{exercise}
...
This is interesting.
Why is the difference in the waiting time so large between what we get when we simulate 100 jobs and what we should (?)
get according to Sakasegawa's formula? Since we simulate the $M/M/1$ queue here, Sakasegawa's result is exact.

\begin{exercise}
Run an example with more jobs, 1000 or so. Does the average  sojourn time in the simulation converge to the theoretical result?
\end{exercise}

\begin{exercise}
Another idea is this. We start with an empty system, and that is a very particular state. Suppose we would start with 10 jobs in queue. Do a simulation for 100 jobs, but with 10 jobs in queue at the start. What is then the average sojourn time?  Hint, below I include some  code to put 10 jobs in queue right at the start.
\end{exercise}
