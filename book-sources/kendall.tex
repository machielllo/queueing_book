...
The most important inter-arrival and service distributions are the exponential distribution, denoted with the shorthand $M$\sidenote{which stands for Markov or Memoryless}, and the general distribution,  denoted with $G$\sidenote{with the implicit assumption that the first moment is finite}. We write $D$ for a deterministic (constant) random variable.
