...
S.mean()
#+end_src
Explain that we need to set ~S[0]=0~ to get the correct result. If ~num~ is big number, does it matter that we set ~S[0]=0~?
#+begin_hint
Did we really serve job 0?
#+end_hint
#+end_exercise
...
#+begin_src shell
mv queues-simulations-in-continuous-times.pdf ../pdf-files/
#+end_src
